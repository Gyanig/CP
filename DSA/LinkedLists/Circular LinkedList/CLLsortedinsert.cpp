...
Node* sortedinsert(Node* tail,int new_data)
{
	Node* new_node=new Node();
	new_node->data=new_data;
	
	Node* n=tail->next;
	if(tail->data <=new_data && (tail->next)->data < new_data)
	{
		new_node->next=tail->next;
		tail->next=new_node;
		tail=new_node;
	...
	else if((tail->next)->data >= new_data)
	{
		new_node->next=tail->next;
		tail->next=new_node;
		return tail;
		...
		do
		{
			if(n->data <= new_data && (n->next)->data > new_data)
			{
...
int main()
{
	...
	int new_data=0;
	cout<<"Enter the a data to insert sorted in the CLL"<<endl;
	cin>>new_data;
	
	tail= sortedinsert(tail,new_data);
